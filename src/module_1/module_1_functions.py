...
def generic_API_call (url: str, cooldown = 10,tries = 3, params: dict = None) -> dict | None : 
...
def validate_schema (answ: dict ):
    ...
    daily_props = {var: {"type": "array", "items": {"type": "number"}} for var in vars.VARIABLES}
    ...
    schema = {
        ...
        "properties": {
             "daily": { # debe existir la sección 'daily'
                ...
                "required": ["time"] + vars.VARIABLES  # todas las variables son obligatorias + tiempo 
            },
...
def process_city_data (city:str,data:dict,units_ret:dict) -> tuple[pd.DataFrame,dict]:
    #recive un diccionario de ciudad, datos y generamos una matriz a partir de ello

    # cojemos las unidades i las tratamos para que no haya conflicto de nombres en la matrix
    ...
    matrix["time"] = pd.to_datetime(matrix["time"]) #ponemos los tiempos en formato tiempo 
    matrix["city"] = city
...
def means_city_data (matrix: pd.DataFrame) -> pd.DataFrame:
    #recive una matriz con datos diarios i devuelve una matriz solo con los meses y años y su media 

    monthly_mean_matrix = matrix.resample("ME", on="time").agg({ # ME indica que lo haga por meses en la columna tiempo
    ...
    monthly_mean_matrix["time"] = monthly_mean_matrix["time"].dt.strftime("%Y-%m") #dt = datetime operaciones de pandas
...
def unite_matrix (matrix_list: list[pd.DataFrame]) -> list [pd.DataFrame] : 
    # creamos 3 matrizes nuevas, cada una con 3 columnas, una de las temperaturas, otra del viento, etc.
    ...
    for i in range (len(matrix_list)):
        ...
        city_name = matrix_list[i]["city"].iloc[0]  # toma el nombre de ciudad como string
...
def plot_info(data_list: list[pd.DataFrame], variable_names: list[str],units:dict) -> None:
