...
def test_generic_API_call(monkeypatch):
    ...
    monkeypatch.setattr("requests.get", mock_get) # informa de que se use un get falso cuando se hace request.get
...
def test_get_data_meteo_api(monkeypatch):
    ...
    monkeypatch.setattr(func, "generic_API_call", lambda url, cooldown, tries, params: var.mock_data) # cada vez que hacemos llamada a generci api call asumimos que nos devulve mock data 
    monkeypatch.setattr(func, "validate_schema", lambda answ: True) # cada vez que se valida el schema asumimos que es valido 

    result = func.get_data_meteo_api(52.52, 13.41) # le damos las cordenadas que tenemos en mock data
    assert result == var.mock_data # comporvamos que no haya habido ninguna distorion de los datos 
...
def test_means_city_data():
    ...
    assert monthly_df["temperature_2m_mean"].iloc[0] == pytest.approx(df["temperature_2m_mean"].mean()) # comprueba que la media coicida 
...
def test_unite_matrix():
    ...
    assert len(result) == 3
    assert all(isinstance(x, pd.DataFrame) for x in result) #compreba que cada elemento de salida es un dataframe 
